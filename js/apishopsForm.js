...
jQuery.fn.apishopsForm=function(options) 
{
   ...
   /**
    * Восстанавливает необходимые параметры, получаем цены и списки
   ...
   function check()
   {
        ...
        if(typeof settings.featured != 'undefined'){
            ...
            if(typeof settings.featured.theme == 'undefined')
                settings.featured.theme=1;                  

            if(typeof settings.featured.type == 'undefined')
                settings.featured.type=settings.type; 
            ...
            if(typeof settings.featured.container == 'undefined' || $(settings.featured.container).length==0){
                debugAlert('Контейнер "'+settings.featured.container+'"(параметр settings.featured.container), в котором будут размещаться формы заказа дополнительных товаров не задан')
   ...
   {            
            /**
             * Все jquery формы воспроизводим, клонируем и берем за образец, обрабатываем и забываем
             * @type {Number}
             */
            
            if(settings.form!='light' && settings.form!='normal'){
                var formexample=settings.form;
                var code=$(settings.form).html()
                settings.form=$(settings.form).html(code).clone() 
                $(formexample).remove();              
            }                        
            ...
            if(settings.form=='normal'){
                settings.form=$(apishopsFormTemplates[settings.theme].theme_main_normal).clone();    
                settings.form_type='normal';
            }
            else if(settings.form=='light'){                
                settings.form=$(apishopsFormTemplates[settings.theme].theme_main_light).clone();      
                ...
                if(typeof inputs_tmp['fio']=='undefined'  || typeof inputs_tmp['phone']=='undefined'  || typeof inputs_tmp['address']=='undefined'  || typeof inputs_tmp['count']=='undefined' ){
                    ...
                    settings.form=$(settings.form).clone().css('display',"");
   ...
   function loadRenderBind()
   {
        if(typeof settings.form.attr('id')=='undefined')
            settings.form.attr('id','apishopsId'+String.fromCharCode(65 + Math.floor(Math.random() * 26)) + Date.now())
   ...
   function render(){
        ...
        $.each( settings.containers, function( index, value ) {
            ...
            apishopsLog(index + ": " + value +'?'+settings.displayed_containers+' is '+settings.displayed_containers.indexOf(index));
            if(typeof $container !='undefined' && $container.length && typeof  settings.displayed_containers!='undefined' && settings.displayed_containers.indexOf(index)>=0)
            {
        ...
        for(index in settings.inputs){
            ...
            if($input.length &&  typeof $input !='undefined' && typeof  settings.hidden_fields!='undefined' && settings.hidden_fields.indexOf(index)>=0)
            {
                $input.hide();
            }
        }    

        $('.__NAME__', settings.form).html(settings.name)
        $('.__DESC__', settings.form).html(settings.description)
        $('.__IMG__', settings.form).attr('src',settings.img)
        $('.__PRICE__', settings.form).html(Math.round(settings.price))
        $('.__OLDPRICE__', settings.form).html(Math.round(settings.oldprice))
        $('.__NAME__', settings.form).html(settings.name)
        if(settings.lang==6){
            $('.__CYR__', settings.form).html('грн')
            $('.__CY__', settings.form).html('грн')
        }
        else{
            $('.__CYR__', settings.form).html('руб')
            $('.__CY__', settings.form).html('р')               
        }             
   ...
   /**
    * Функция отображения отладочных сообщений
    * (при включенном settings.debug)
    * @param  {[type]} message [description]
    * @return {[type]}         [description]
    */
   function debugAlert(message){
        if(typeof settings.debug != 'undefined' && settings.debug)
            alert(message)
   }
